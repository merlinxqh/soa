...
import com.hiveview.admin.icp.commom.RedisKeyConstants;
...
public class SystemUserUtils {
	...
	private static SysResourceApiService resourceApiService = SpringContextHolder.getBean(SysResourceApiService.class);
	...
	public static List<SysResourceDto> getResourceList(){
    	...
    	if(null != user && null != user.getDefaultRole()){
    		...
    		if(redisService.exists(resourceKey)){
    			...
    			//调用rpc获取该用户权限数据 TODO
                resourceList=resourceApiService.getResourceByRole(user.getDefaultRole());
